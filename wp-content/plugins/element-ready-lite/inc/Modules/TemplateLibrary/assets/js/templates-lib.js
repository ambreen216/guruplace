...
;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('firefly/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateFireEyeView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-eready',
		ui: function ui() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			fe_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && fe_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-eready',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'firefly' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-eready',
		...
		ui: {
			...
			clickFilter: '.element-ready-template-library-cat-text',
		...
		events: {
			...
			'click @ui.clickFilter': 'onTextFilterClick',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateFireEyeView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = fe_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = fe_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : fe_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(fe_templates_lib.templates.getFilter('text') || fe_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('firefly' === fe_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('firefly' === fe_templates_lib.templates.getFilter('source') && 'page' !== fe_templates_lib.templates.getFilter('type')) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			fe_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onTextFilterClick: function onTextFilterClick(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
				var text = $select.data('elementor-text');
			
			fe_templates_lib.templates.setFilter(filterName,text);
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
				
			fe_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			fe_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo_eready',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-eready',
		...
		onSyncClick: function() {
			...
			fe_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('firefly');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('firefly').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getTemplateActionButton: function(templateData) {
			...
			if ( templateData.isPro ) {
				viewId = '#tmpl-elementor-template-library-get-pro-button-eready';
			}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'firefly' ) ) {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('eready_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			//var activeSource = 'fireflyserver';
			return templatesCollection.filter(function (model) {
				if ('firefly' !== model.get('source')) {
					return false;
				}

				return 'footer' === model.get('type') || 'header' === model.get('type') || 'block' === model.get('type') || 'page' === model.get('type');
			});
		};
	...
	fe_templates_lib.templates = new TemplateLibraryManager();
	fe_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			...
			btnStyle += '.elementor-add-section-area-button.element-ready-add-template-button img { height: 40px; }';
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			...
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.element-ready-add-template-button' ).remove();
